...
void tee_sha1_Transform(uint32_t state[5], const uint8_t block[SHA1_BLOCK_SIZE]) {
  uint32_t a, b, c, d, e;
  uint32_t w[80];
...
void tee_sha1_Update(SHA1_CTX *context, const uint8_t *data, size_t len) {
  size_t i;
  ...
  for (i = 0; i + SHA1_BLOCK_SIZE <= len; i += SHA1_BLOCK_SIZE) {
    tee_sha1_Transform(context->state, data + i);
  }
...
void tee_sha1_Final(SHA1_CTX *context, uint8_t digest[SHA1_DIGEST_SIZE]) {
  ...
  if (index > SHA1_BLOCK_SIZE - 8) {
    ...
    tee_sha1_Transform(context->state, context->buffer);
    memset(context->buffer, 0, SHA1_BLOCK_SIZE);
  } else {
